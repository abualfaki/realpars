...
WITH source AS (
    SELECT *
    FROM {{ source('raw_cc_data', 'post_liked') }}
    ...
    {% if is_incremental() %}
        ...
        WHERE safe_cast(created_at AS timestamp) >= TIMESTAMP_SUB(
            ...
            -- when a course.lesson.completed event is back populated. Futhermore, in 
            -- the documentationthere are no  examples of circumstnacesthat will require 
            -- the need to back populate data.

            -- Since the Data in circle_community_raw_datsets.post_liked is 
...
base_data as (
    SELECT
        ...
        -- Extracting fields from the JSON payload initiator object (person who liked the post)
        ...
        json_value(payload, '$.posts_basic.id') as post_id,
...
clean_base_data as (
    SELECT
        -- Initiator fields (person who liked the post)
        initiator_type,
